...
EC_INT ECTCPSocket::Connect(ECSocketAddress *pDstAddress)
{
    EC_INT nRet = 0;
    if (pDstAddress)
    {
        ...
        addrSrv.sin_port = htons(pDstAddress->nPort);
        addrSrv.sin_addr.S_un.S_addr = inet_addr(pDstAddress->strIP.ToCStr());
        ...
        address.sin_port = htons(pDstAddress->nPort);
        address.sin_addr.s_addr = inet_addr(pDstAddress->strIP.ToCStr());
...
ECTCPSocket ECTCPSocket::Accept(ECSocketAddress *pFromAddress)
{
    EC_INT socket = -1;
    ...
    if (pFromAddress)
    {
        ...
        pFromAddress->nPort = address.sin_port;
